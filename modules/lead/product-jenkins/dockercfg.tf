locals {
  ...
  harbor_pass       = var.enable_harbor ? harbor_robot_account.robot[0].secret : ""
  harbor_robot_name = var.enable_harbor ? harbor_robot_account.robot[0].full_name : ""
  config_json = templatefile("${path.module}/dockercfg.tpl", {
    ...
    harbor_auth = base64encode(
      "${local.harbor_robot_name}:${local.harbor_pass}",
    )
